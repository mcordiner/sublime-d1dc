...
exit(1);
  ...
  if(strncmp(string, expectedLine, strlen(expectedLine))!=0 && strncmp(string, expectedLine2, strlen(expectedLine))!=0){
    if(!silent){
        ...
        if(par->lte_only){
          ...
          for(itrans=0;itrans<md[i].part[k].ntrans;itrans++){
            ...
            for(itemp=0;itemp<md[i].part[k].ntemp;itemp++){
              ...
              md[i].part[k].down[j] *= par->colliScale; /* User scaling of collisional rates */
            }
            checkFscanf(fscanf(fp,"\n"), 0, "LF");
          }
        } /* End if(par->lte_only) */
...
void setUpGir(configInfo *par, molData *md){
  ...
  for(i=0;i<par->nSpecies;i++){
    ...
    if((fp=fopen(par->girdatfile[i], "r")) != NULL){
      while (fscanf(fp, "%d %d %lf", &ilev, &jlev, &pumpRate) != EOF){
        if(ilev<=0 || jlev<=0){
          if(!silent){
...
exit(1);
        ...
        md[i].gir[(ilev-1)*md[i].nlev+jlev-1] = pumpRate * par->girScale;
      }
      fclose(fp);
    }else{
      snprintf(message, STR_LEN_0, "Girdat file %s cannot be opened.",par->girdatfile[i]);
      bail_out(message);
      exit(1);
      }
  }
