...
/*....................................................................*/
// void
// calcLineAmpSample(const double binv, double *projVels, const int nSteps\
...
/*....................................................................*/
void 
traceray(double *xs, int ppi, imageInfo * img,configInfo * par, struct grid * gp, double *dx, molData * md, struct radius_struct radius_struct, int im){

  int ichan,molI,lineI,i,posnu,posnl,posn,index;
  double x,y,z, jnu, alpha, vThisChan, r, dz,lineRedShift,deltav,specNumDensl,specNumDensu,x0, x1, y0, y1,vfac,dtau,remnantSnu,expDTau,brightnessIncrement,contJnu,contAlpha;
  _Bool insideImage = 1;
        ...
        if (r <= radius_struct.radius[0] || r >= radius_struct.radius[par->pIntensity - 1])
          ...
          for (i = 0; i < par->pIntensity; i++) {
            if (radius_struct.radius[i] > r) {
              posnu = radius_struct.id[i];
              posnl = radius_struct.id[index - 1];
              break;
            }
          }
          if(fabs(radius_struct.radius[index]-r) < fabs(r-radius_struct.radius[index-1]))
            posn = posnu;
          else
            posn = posnl;
...
Note that the argument 'md', and the grid element '.mol', are only accessed for line images.
  */
  ...
  double pixelSize, imgCentreXPixels, imgCentreYPixels, minfreq, absDeltaFreq, xs[2];
  double dx[DIM],oneOnNumRays,scale;
  unsigned int totalNumImagePixels, ppi,xi,yi;
  int ichan,lastChan,minNumRaysForAverage=2;;
  int gi, molI, lineI, i,j, di, pixel_x, pixel_y, id,index;
  int cmbMolI, cmbLineI; 
  double local_cmb, cmbFreq;
  ...
  if (img[im].doline) {
    ...
    if (img[im].bandwidth > 0 && img[im].velres > 0) {
      ...
      /*(img[im].velres > 0 && img[im].nchan > 0 */
  ...
  */
  if (img[im].doline) {
    if (img[im].trans >= 0) {
      ...
      /* User didn't set trans. Find the nearest line to the image frequency. */
  ...
  pixel_y = 0;
  for (ppi = 0; ppi<totalNumImagePixels; ppi++){
    pixel_x = ppi % img[im].pxls;
    if (pixel_x == 0 && ppi != 0)
      pixel_y += 1;

    xs[0] = (0.5 + pixel_x - imgCentreXPixels) * pixelSize;
    xs[1] = (0.5 + pixel_y - imgCentreYPixels) * pixelSize;

    if(abs(xs[0])>pixelSize*nsupsamppix || abs(xs[1])>pixelSize*nsupsamppix){
      img[im].pixel[ppi].numRays++;
      traceray(xs,ppi,img,par,gp,dx,md,radius_struct,im);
    }
  }
  ...
  for(j=1-(nsupsamppix*supsamp);j<(nsupsamppix*supsamp);j++){
    for(i=1-(nsupsamppix*supsamp);i<(nsupsamppix*supsamp);i++){
      xs[0] = j*scale;
      xs[1] = i*scale;
      xi = floor(xs[0]/pixelSize + imgCentreXPixels);
      yi = floor(xs[1]/pixelSize + imgCentreYPixels);
      ppi = yi*(unsigned int)img[im].pxls + xi;
      img[im].pixel[ppi].numRays++;
      traceray(xs,ppi,img,par,gp,dx,md,radius_struct,im);
    }
  }
  ...
  if (par -> polarization) {
    /* just add cmb to Stokes I, which is the first 'channel' */
    lastChan = 0;
  } else {
